...
[View the live demo on Heroku](https://react-redux-boilerplate-101.herokuapp.com/) or the [individual UI components](https://ivanarielcaceres.github.io/react-redux-training) of my Storybook on Github Pages.
...
- Storybook of [my UI components](https://ivanarielcaceres.github.io/react-redux-training)
- Travis CI to build the production bundles and deploy it to Heroku, run the tests, creating and publishing [the test coverage report](https://coveralls.io/github/ivanarielcaceres/react-redux-training) and the [UI documentation as a Storybook](https://ivanarielcaceres.github.io/react-redux-training) on Github Pages.
...
- Download this boilerplate repository as a zip:
  https://github.com/ivanarielcaceres/react-redux-training/archive/master.zip
...
- Replace `react-redux-training` with `$REPOSITORY_NAME` and `ivanarielcaceres` with `$YOUR_GITHUB_USERNAME` on `package.json`
- Replace `react-redux-training` with `$REPOSITORY_NAME` on `README.md` (so the links will not be broken)
...
```
travis encrypt MY_HEROKU_AUTH_TOKEN -r ivanarielcaceres/react-redux-training
```
